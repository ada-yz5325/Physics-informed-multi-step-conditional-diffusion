...
class LossParams(object):
    def __init__(
        ...
        self.extraMSEvelZ = extraMSEvelZ # extra mse weight for z velocity in 3D
        ...
        self.regVort      = regVort      # vorticity regularization weight
    ...
    def fromDict(cls, d:dict):
        p = cls()
        p.recMSE       = d.get("recMSE", 1.0)
        p.recLSIM      = d.get("recLSIM", 0)
        p.predMSE      = d.get("predMSE", 1.0)
        p.predLSIM     = d.get("predLSIM", 0)
        p.extraMSEvelZ = d.get("extraMSEvelZ", 0)
        p.regMeanStd   = d.get("regMeanStd", 0)
        p.regDiv       = d.get("regDiv", 0)
        p.regVort      = d.get("regVort", 0)
        p.regVae       = d.get("regVae", 0)
        p.regLatStep   = d.get("regLatStep", 0)
        return p
